...
{
/**
 * `std::sort` index with custom comparator that uses values contents instead of index contents. Then use
 * `boost::algorithm::apply_permutation()` to sort values. This approach doesn't require calling `std::sort()`
 * twice, it uses the information from `std::sort`ing the permutation index to sort values.
 *
 * `boost::algorithm::apply_permutation()` destroys the permutation index in the process of applying it.
 * This algorithm therefore has to make a copy of the index to give it to `apply_permutation()`.
 *
 * Boost's `boost::algorithm::apply_permutation()` is inspired by the
 * https://devblogs.microsoft.com/oldnewthing/20170102-00/?p=95095 article and the following articles.
 *
...
void boost_index_apply_sort(RandomIt1 value_begin, RandomIt1 value_end, RandomIt2 index_begin, RandomIt2 index_end, Compare cmp)
{
